...
/// struct for typed errors of method [`create_orders`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum CreateOrdersError {
...
/// struct for typed errors of method [`options_orders`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum OptionsOrdersError {
    Status401(models::Orders401Response),
    Status500(models::Orders500Response),
    UnknownValue(serde_json::Value),
}
...
pub async fn create_orders(configuration: &configuration::Configuration, x_account: &str, create_orders_request: models::CreateOrdersRequest) -> Result<models::Orders200Response, Error<CreateOrdersError>> {
    ...
    let p_create_orders_request = create_orders_request;
    ...
    req_builder = req_builder.json(&p_create_orders_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::Orders200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::Orders200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<CreateOrdersError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Options method is used to describe the communication options for the targeted resource.
pub async fn options_orders(configuration: &configuration::Configuration, ) -> Result<(), Error<OptionsOrdersError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<OptionsOrdersError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
