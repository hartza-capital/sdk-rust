...
/// struct for typed errors of method [`options_search_news`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum OptionsSearchNewsError {
...
pub async fn options_search_news(configuration: &configuration::Configuration, ) -> Result<(), Error<OptionsSearchNewsError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<OptionsSearchNewsError> = serde_json::from_str(&content).ok();
...
pub async fn search_news(configuration: &configuration::Configuration, v1_screener_query_request: Option<models::V1ScreenerQueryRequest>) -> Result<models::SearchNews200Response, Error<SearchNewsError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::SearchNews200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::SearchNews200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<SearchNewsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
