...
pub async fn currency(configuration: &configuration::Configuration, ticker: &str) -> Result<models::V1CurrencyResponse, Error<CurrencyError>> {
    ...
    let uri_str = format!("{}/v1/currency/{ticker}", configuration.base_path, ticker=crate::apis::urlencode(p_ticker));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::V1CurrencyResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::V1CurrencyResponse`")))),
        }
    } else {
        let content = resp.text().await?;
