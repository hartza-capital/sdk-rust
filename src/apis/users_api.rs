...
/// struct for typed errors of method [`options_user`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum OptionsUserError {
...
pub async fn options_user(configuration: &configuration::Configuration, ) -> Result<(), Error<OptionsUserError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<OptionsUserError> = serde_json::from_str(&content).ok();
...
pub async fn user(configuration: &configuration::Configuration, ) -> Result<models::User200Response, Error<UserError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::User200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::User200Response`")))),
        }
    } else {
        let content = resp.text().await?;
