...
/// struct for typed errors of method [`options_time`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum OptionsTimeError {
...
pub async fn options_time(configuration: &configuration::Configuration, ) -> Result<(), Error<OptionsTimeError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<OptionsTimeError> = serde_json::from_str(&content).ok();
...
pub async fn time(configuration: &configuration::Configuration, timezone: Option<&str>) -> Result<models::Time200Response, Error<TimeError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::Time200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::Time200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<TimeError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
