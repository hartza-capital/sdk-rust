...
/// struct for typed errors of method [`options_filings_by_cik`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum OptionsFilingsByCikError {
    UnknownValue(serde_json::Value),
}
...
pub async fn filings_us(configuration: &configuration::Configuration, proxy: &str) -> Result<models::FilingsUs200Response, Error<FilingsUsError>> {
    ...
    let uri_str = format!("{}/v1/filings/us/{proxy}", configuration.base_path, proxy=crate::apis::urlencode(p_proxy));
...
pub async fn options_filings_by_cik(configuration: &configuration::Configuration, proxy: &str) -> Result<(), Error<OptionsFilingsByCikError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_proxy = proxy;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<OptionsFilingsByCikError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
